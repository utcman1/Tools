...
int sendbulk(int fd, struct sockaddr *to, int tolen)
{
	const int vlen = 128;
	struct mmsghdr hdr[vlen];
	struct iovec iov;
	iov.iov_base = payload;
	iov.iov_len = mtu;

	for (int i = 0; vlen > i; ++i)
	{
		hdr[i].msg_hdr.msg_name = to;
		hdr[i].msg_hdr.msg_namelen = tolen;
		hdr[i].msg_hdr.msg_iov = &iov;
		hdr[i].msg_hdr.msg_iovlen = 1;
		hdr[i].msg_hdr.msg_control = NULL;
		hdr[i].msg_hdr.msg_controllen = 0;
		hdr[i].msg_hdr.msg_flags = 0;
		hdr[i].msg_len = 0;
	}

	int ret = sendmmsg(fd, hdr, vlen, 0);
...
void flood(int fd, struct sockaddr *to, int tolen)
{
	...
	for (pkt = 0; pkt < (unsigned long long)count; pkt+=128) {
		int nsend = sendbulk(fd, to, tolen);
		totbit += (nsend + 28) * 8;
	}
